...
const DialogAttachment = ({
  ...
  const fetchFileData = async (data: QUOTATION_ALL | MS_CUSTOMER_ALL) => {
    if (data.image_url) {
    ...
    if (isOpen) {
      if (!isChangeFile) {
        if (quotationData) {
          ...
          fetchFileData(customerData);
        }
      }
    }
  }, [quotationData, customerData, isOpen, isChangeFile]);
