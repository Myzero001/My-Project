...
export default function SupplierDeliveryNoteAddListFeatures({ repairReceiptId, onSubmitSuccess }: Props) {
    ...
    const fetchData = () => {
        if (selectedOption && sndId) {
            ...
            Promise.all([
                getBySDNandRRidSelect(sndId, selectedOption),
                getRepairReceiptListRepairByRepairReceiptId(selectedOption),
            ])
                .then(([response2, response]) => {
                    console.log("ข้อมูลที่เคยมีก่อนหน้า: ", response2.responseObject);
                    console.log("RepairReceiptListRepair:", response.responseObject);
                    ...
                    const findPreviousItem = (idRepairList: string) =>
                        previousData.find((prevItem: any) => prevItem.master_repair_id === idRepairList);

                    const repairReceiptData: RepairReceipt = {
                        id: selectedOption || "",
                        repairList: repairReceiptList.map((item: any) => {
                            const prevItem = findPreviousItem(item.master_repair.master_repair_id);
                            console.log("prevItem: ", prevItem);
                            return {
                                status: prevItem ? true : false,
                                idRepairList: item.master_repair.master_repair_id || "",
                                receiptListName: item.master_repair.master_repair_name,
                                price: prevItem ? prevItem.price : item.price ?? 0, // ถ้ามีข้อมูลเก่าให้ใช้
                                qty: prevItem ? prevItem.quantity : 1, // ถ้ามีข้อมูลเก่าให้ใช้
                                total: (prevItem ? prevItem.price * prevItem.quantity : item.price ?? 0 * 1),
                            };
                        }),
                    };
    ...
    const onSubmit = async () => {
        ...
        const payload = {
            ...
            repairList: selectedItems.map(item => ({
                status: item.status,
