...
type DialogAttachmentProps = {
  ...
  errorMessage?: string;
  quotationData: QUOTATION_ALL | MS_CUSTOMER_ALL |MS_USER_ALL| undefined;
...
const DialogAttachment = ({
  ...
  const fetchFileData = async (data: QUOTATION_ALL | MS_CUSTOMER_ALL | MS_USER_ALL) => {
    ...
    // ตรวจสอบว่ามี image_url หรือ employee_image
    let imageField: string | undefined;
  
    if ("image_url" in data && typeof data.image_url === "string") {
      imageField = data.image_url;
    } else if ("employee_image" in data && typeof data.employee_image === "string") {
      imageField = data.employee_image;
    }
  
    if (imageField) {
      const image_urls = imageField.split(",");
  
      if (image_urls.length > 0) {
        preData.image_url = await Promise.all(
          image_urls.map(async (image) => {
            const response = await fetchFileByURL(image);
            const responseFullbanner_image_urls = await fetchImages(response.responseObject);
  ...
  //     if(!isChangeFile){fetchFileData(customerData)}
  //   }
  // },[isOpen,customerData])
              ...
              {isLoadingUploadFile || isLoadingData ? (
                ...
                <>
                  ...
                  >
                    ...
                    {disable && images && images?.length <= 0 ? (
                      <EmptyImage />
                    ) : (
                      images &&
                      images?.length > 0 && (
