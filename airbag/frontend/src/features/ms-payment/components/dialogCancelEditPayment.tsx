...
import { cancelPaymentEdits } from "@/services/ms.payment.service";
...
type DialogCancelEditPaymentProps = {
  data: PAYMENTTYPE | undefined;
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (payload: approveQuotationType) => void;
};
const DialogCancelEditPayment = (props: DialogCancelEditPaymentProps) => {
  ...
  const onSubmitHandler = async (payload: approveQuotationType) => {
    ...
    if (data?.id) {
      cancelPaymentEdits(data?.id, payload.remark)
        .then(() => {
          showToast("ยกเลิกการขอแก้ไขใบชำระเงินเรียบร้อยแล้ว", true);
          handleConfirm(payload);

          reset();
        })
        .catch(() => {
          showToast("ยกเลิกแก้ไขใบชำระเงินไม่สำเร็จ", false);
...
export default DialogCancelEditPayment;
