...
export const supplierRepairReceiptListRepository = {
    ...
    count: async (companyId: string, searchText: string) => {
        const searchPrice = parseFloat(searchText);
        return await prisma.supplier_repair_receipt_list.count({
            where: {
                company_id: companyId,
                ...(searchText && {
                    OR: [
    ...
    create: async (companyId: string, userId: string, payload: TypePayloadSupplierRepairReceiptList) => {
        ...
        try {
            const newReceiptList = await prisma.supplier_repair_receipt_list.create({
                data: {
                ...
                include: {
                    supplier_repair_receipt: { // ข้อมูล Header ใบรับซ่อม
                        include: {
                    ...
                    supplier_delivery_note: { // ข้อมูลใบส่งของต้นทาง
                        include: {
                    ...
                    supplier_repair_receipt_list_created_by_user: { // ข้อมูลคนสร้างรายการนี้
                        select: { employee_id: true, first_name: true } // เลือกเฉพาะ field ที่จำเป็น
                    },
                    supplier_repair_receipt_list_updated_by_user: { // ข้อมูลคนอัปเดตรายการนี้
                        select: { employee_id: true, first_name: true } // เลือกเฉพาะ field ที่จำเป็น
                    },
    ...
    findByIdAsync: async (companyId: string, id: string) => {
        return await prisma.supplier_repair_receipt_list.findFirst({
            where: {
                company_id: companyId,
                id: id,
            },
    ...
    update: async (
        ...
        userId: string,
        id: string,
        payload: TypePayloadSupplierRepairReceiptList
    ) => {
        const calculatedTotalPrice = payload.price && payload.quantity 
            ? payload.price * payload.quantity 
            : payload.price || 0;

        return await prisma.supplier_repair_receipt_list.update({
            where: {
                id: id,
                company_id: companyId,
            },
    ...
    delete: async (companyId: string, id: string) => {
        return await prisma.supplier_repair_receipt_list.delete({
            where: {
                id: id,
                company_id: companyId,
            },
    ...
    checkIfReferencedInSendForClaim: async (companyId: string, supplierRepairReceiptListId: string): Promise<boolean> => {
        const count = await prisma.send_for_a_claim_list.count({
            where: {
                company_id: companyId,
    ...
    updateFinishStatus: async (
        ...
        return await prisma.supplier_repair_receipt_list.updateMany({
            where: {
                company_id: companyId,
    ...
    findPayloadListById: async (
        ...
        try {
            const receipts = await prisma.supplier_repair_receipt.findMany({
                where: {
                    company_id: companyId,
    ...
    updateFinishStatusBySupplierDeliveryNoteRepairReceiptListId: async (
        companyId: string,
        userId: string,
        supplierDeliveryNoteRepairReceiptListId: string,
        masterRepairId: string,
        masterRepairReceiptId: string,
        isFinish: boolean,
        finish_by_receipt_doc: string
    ) => {
        return await prisma.supplier_repair_receipt_list.updateMany({
